"""
Phase 2 v2: Entity Resolution & Consolidation with Regional Variant Detection
Consolidate TRUE duplicates while keeping regional variants SEPARATE
"""
...
class Phase1DataLoader:
    """Load and combine all Phase 1 outputs."""
    ...
    def _combine_data(self) -> pd.DataFrame:
        ...
        if not self.risk_data.empty:
            risk_cols = self.risk_data.columns
            merge_key = "Vendor Name" if "Vendor Name" in risk_cols else "supplier_name"
            
            if merge_key in risk_cols:
                self.risk_data = self.risk_data.rename(columns={merge_key: "Vendor Name"})
                df = df.merge(self.risk_data, on="Vendor Name", how="left")
        
        if not self.transactions.empty:
            trans_counts = self.transactions.groupby("Vendor_Name" if "Vendor_Name" in self.transactions.columns else "supplier").size().reset_index(name="transaction_count")
            vendor_col = "Vendor Name" if "Vendor Name" in df.columns else "name"
            trans_counts.columns = [vendor_col, "transaction_count"]
            df = df.merge(trans_counts, on=vendor_col, how="left")
            df["transaction_count"] = df["transaction_count"].fillna(0).astype(int)
        
        if not self.disruptions.empty:
            disruption_cols = self.disruptions.columns
            vendor_col_disr = "Vendor_Name" if "Vendor_Name" in disruption_cols else "vendor_name"
            
            if vendor_col_disr in disruption_cols:
                disruption_counts = self.disruptions.groupby(vendor_col_disr).size().reset_index(name="disruption_count")
                disruption_counts = disruption_counts.rename(columns={vendor_col_disr: "Vendor Name"})
...
# ============================================================
# STEP 2: REGIONAL VARIANT DETECTION (NEW!)
# ============================================================
...
class RegionalVariantDetector:
    ...
    def classify_relationship(name1: str, name2: str) -> Tuple[str, Dict]:
        ...
        Returns: (relationship_type, details_dict)
        """
        # Extract regions and base names
        ...
        # Case 1: Both have no regions - normal duplicates check
        ...
        # Case 2: Both have regions, same base - regional variants
        ...
        # Case 3: One has region, one doesn't - potential parent/subsidiary
        if base1 == base2 and bool(regions1) != bool(regions2):
            # One is global, one is regional
        ...
        # Case 4: Different base names - not duplicates
...
class SmartFuzzyMatcher:
    ...
    def find_duplicates(vendors: List[str], threshold: float = 0.85) -> List[Dict]:
        ...
        Returns list of duplicate records with classification.
        """
        duplicates = []
        ...
        for i in range(len(vendors)):
            for j in range(i + 1, len(vendors)):
                ...
                # Only consider for consolidation if NOT regional variant
...
# ============================================================
# STEP 4: BUILD CLUSTERS (Same as v1)
# ============================================================
...
class MetricsMerger:
    ...
    def merge_variant_metrics(self, master_name: str, variant_names: List[str]) -> Dict:
        ...
        for record in records:
            perf = record.get("PERF_SCORE", record.get("Perf_Score"))
            if pd.notna(perf):
        ...
        fin_stabs = []
        for record in records:
            fin = record.get("FIN_STAB", record.get("Fin_Stab"))
            if pd.notna(fin):
        ...
        if perf_score and perf_score >= 85:
            master["Risk_Category"] = "Low"
        elif perf_score and perf_score >= 70:
...
class RegionalVariantReporter:
    ...
    def report_regional_variants(self, all_vendors: List[str]) -> pd.DataFrame:
        ...
        # Report variants (same base, different regions)
        for base_name, vendors in vendor_groups.items():
            if len(vendors) > 1:
                ...
                if len(set(regions_set)) > 1:
                    # Different regions - these are variants
                    for v in vendors:
    ...
    def build_unified_master_table(self) -> pd.DataFrame:
        ...
        for cluster_id, master_name in self.masters.items():
            ...
            record = {
                ...
                "Country": consolidated.get("Land1", consolidated.get("Country")),
                "Perf_Score": consolidated.get("Perf_Score"),
                "Fin_Stab": consolidated.get("Fin_Stab"),
                "Risk_Category": consolidated.get("Risk_Category"),
                "SupplyRiskScore": consolidated.get("SupplyRiskScore"),
                "ProfitImpact": consolidated.get("ProfitImpactScore"),
...
class Phase2Validator:
    ...
    def validate(self, clusters: Dict, masters: Dict, unified_table: pd.DataFrame) -> Dict:
        ...
        # Check 3: Metrics in valid range
        for idx, row in unified_table.iterrows():
            perf = row.get("Perf_Score")
            if pd.notna(perf) and (perf < 0 or perf > 100):
                errors.append(f"{row['MasterName']}: Invalid Perf_Score {perf}")
...
def main():
    ...
    try:
        ...
        vendor_col = "Vendor Name" if "Vendor Name" in combined_data.columns else "name"
        all_vendors = combined_data[vendor_col].unique().tolist()
