...
INTENTS = {
    ...
    "top_vendors":    ["top.*vendor","top.*supplier","highest spend","most spend",
...
def resp_help() -> str:
    return """**SRSID Procurement Assistant — What I can answer:**

** Summary & Alerts**
- "Give me a supply chain health summary"
- "Which suppliers are high risk right now?"
- "Show disruption alerts for the next 30 days"
- "Which suppliers have negative news sentiment today?"

** Supplier Deep Dives**
- "What is the risk level for [Supplier Name]?"
- "Why is [Supplier Name] categorised as high risk?"
- "Show me delivery performance history for [Supplier Name]"
- "What % of our spend goes to [Supplier Name]?"

** Spend Intelligence**
- "What is our total spend this quarter?"
- "Where is our maverick spend?"
- "Show me spend by risk category"
- "Which suppliers have no long-term contract?"

**️ Operational Performance**
- "Which suppliers are failing on OTIF?"
- "List all suppliers with delays over 5 days"
- "Is there a correlation between news and delivery delays?"

** Alternatives & Comparison**
- "Who is the safest alternative to [High-Risk Supplier]?"
...
- "Should I renew the contract with [Supplier Name]?"
- "What is our concentration risk if we lose [Supplier Name]?"

** What-If Scenarios**
- "What happens if [Supplier Name] goes bankrupt?"
- "If we shift 20% spend from [A] to [B], what changes?"
- "Predict spend for next quarter"

Just ask naturally — I'll find the best answer from your live data. """
...
def resp_whatif(d: dict, q: str) -> str:
    ...
    # Spend shift scenario
    shift_match = re.search(r'(\d+)%?\s*(shift|move|transfer)', ql)
    if shift_match and name:
    ...
    if "predict" in ql and "quarter" in ql:
        ...
        if not qdf.empty and len(qdf) >= 2:
            # Use quarterly transaction data
            last2  = qdf.tail(2)["total_spend"].values
            growth = (last2[-1] - last2[-2]) / last2[-2] if last2[-2] else 0
            predicted = last2[-1] * (1 + growth)
            last_period = qdf.iloc[-1].get("period", "last quarter")
            return (
                f"**Spend Forecast — Next Quarter**\n\n"
                f"Last period ({last_period}) spend: **{fmt_money(last2[-1])}**\n"
                f"Quarter-on-quarter trend: **{growth*100:+.1f}%**\n"
                f"Predicted next quarter: **{fmt_money(predicted)}**\n\n"
                f"*Based on linear trend from {len(qdf)} quarters of transaction data. "
                f"Actual may vary with seasonal patterns and new contracts.*"
            )
        elif not v.empty and "total_annual_spend" in v.columns:
            # Fallback: use annual spend from vendors table
            total_annual  = v["total_annual_spend"].sum()
            quarterly_est = total_annual / 4
            # Check if we have any QoQ signal
            if not qdf.empty and len(qdf) == 1:
                growth_note = "Insufficient quarters for trend — showing flat estimate."
            else:
                growth_note = "No quarterly transaction breakdown yet — run `python ingestion/sap_loader.py` to load transactions."
            return (
                f"**Spend Forecast — Next Quarter (Estimated)**\n\n"
                f"Total annual spend on record: **{fmt_money(total_annual)}**\n"
                f"Estimated quarterly spend: **{fmt_money(quarterly_est)}** (annual ÷ 4)\n\n"
                f"*{growth_note}*\n\n"
                f"For a trend-based forecast, at least 2 quarters of transaction data are needed."
            )
