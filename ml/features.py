...
def load_from_postgres(db: DBClient) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # Vendor master — include spend analytics columns if they exist
    # (populated by ml/spend_analytics.py)
    spend_cols = []
    for col in ["sum_percentage","is_maverick","maverick_type",
                "concentration_risk","diversification_needed",
                "qoq_change_pct","spend_trend","savings_opportunity"]:
        if db.column_exists("vendors", col):
            spend_cols.append(col)

    spend_select = (", " + ", ".join(spend_cols)) if spend_cols else ""
    if spend_cols:
        log.info(f"  Spend analytics columns available: {spend_cols}")
    ...
    tx_vendors = db.scalar(
        "SELECT COUNT(DISTINCT vendor_id) FROM transactions "
        "WHERE vendor_id IS NOT NULL"
    )
    log.info(f"  Transactions in DB: {tx_count:,} rows, {tx_vendors:,} distinct vendors")

    if tx_count and tx_count > 0:
        sample_tx = db.fetch_df(
            "SELECT DISTINCT vendor_id FROM transactions "
            "WHERE vendor_id IS NOT NULL LIMIT 5"
        )
...
def build_geo_industry_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    from config import (COUNTRY_SUPPLY_RISK, COUNTRY_SUPPLY_RISK_DEFAULT,
                        get_country_risk, get_country_risk_tier, INDUSTRY_RISK)

    # Geo risk — now uses evidence-based score per country, not tier buckets
    if "country_code" in df.columns:
        df["geo_risk_numeric"] = df["country_code"].apply(get_country_risk)
        df["geo_risk_tier"]    = df["country_code"].apply(get_country_risk_tier)

        # Log distribution
        tier_dist = df["geo_risk_tier"].value_counts().to_dict()
        avg_score = df["geo_risk_numeric"].mean()
        log.info(f"  Geo risk distribution: {tier_dist}")
        log.info(f"  Avg geo risk score: {avg_score:.3f}")

        # Flag sanctioned-country vendors
        from config import SANCTIONED_COUNTRIES
        df["sanctioned_country"] = df["country_code"].apply(
            lambda c: 1 if str(c).strip().upper() in SANCTIONED_COUNTRIES else 0
        )
        sanctioned_count = df["sanctioned_country"].sum()
        if sanctioned_count > 0:
            log.warning(f"  ⚠️  {sanctioned_count} vendors in sanctioned countries")
    else:
        df["geo_risk_numeric"]  = COUNTRY_SUPPLY_RISK_DEFAULT
        df["geo_risk_tier"]     = "Medium"
        df["sanctioned_country"] = 0

    # Industry risk
    if "industry_category" in df.columns:
        df["industry_risk_numeric"] = (
            df["industry_category"].map(INDUSTRY_RISK).fillna(0.50)
        )
    else:
        df["industry_risk_numeric"] = 0.50
...
def build_composite_risk(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Base risk from geo + industry + delivery + concentration
    ...
    # News boost (up to +15% if active disruptions)
    if "disruption_freq_norm" in df.columns:
        df["composite_risk_score"] = (
            df["composite_risk_score"] * 0.85 +
            df["disruption_freq_norm"].fillna(0) * 0.15
        ).clip(0, 1).round(4)

    # Spend analytics boost (up to +10% if maverick or low SUM)
    spend_risk = pd.Series(0.0, index=df.index)

    if "is_maverick" in df.columns:
        # Maverick vendor (no contract) = higher risk
        spend_risk += df["is_maverick"].fillna(False).astype(float) * 0.5

    if "sum_percentage" in df.columns:
        # Low SUM (<60%) = higher compliance risk
        low_sum = (df["sum_percentage"].fillna(100) < 60).astype(float)
        spend_risk += low_sum * 0.3

    if "concentration_risk" in df.columns:
        # High concentration = higher supply risk
        conc_score = df["concentration_risk"].map(
            {"HIGH": 0.8, "MEDIUM": 0.4, "LOW": 0.1}
        ).fillna(0.1)
        spend_risk += conc_score * 0.2

    # Normalise spend_risk to 0-1
    spend_risk = (spend_risk / spend_risk.max()).fillna(0) \
                 if spend_risk.max() > 0 else spend_risk

    # Apply spend risk as 10% modifier
    spend_cols_present = any(c in df.columns
                             for c in ["is_maverick","sum_percentage","concentration_risk"])
    if spend_cols_present:
        df["composite_risk_score"] = (
            df["composite_risk_score"] * 0.90 +
            spend_risk * 0.10
        ).clip(0, 1).round(4)
        log.info("  Spend analytics signals incorporated into risk score")
