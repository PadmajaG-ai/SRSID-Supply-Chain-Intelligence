...
class DBClient:
    ...
    def execute(self, sql: str, params: tuple = None) -> int:
        ...
        Auto-recovers if the connection is in a failed transaction state.
        ...
        """
        # If a previous statement failed, the transaction is aborted.
        # Roll it back so this statement can run cleanly.
        if self._conn and self._conn.status == psycopg2.extensions.STATUS_IN_TRANSACTION:
            try:
                self._conn.rollback()
                log.debug("Auto-rolled back failed transaction before execute()")
            except Exception:
                pass
